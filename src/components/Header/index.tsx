...
const Header = () => {
  ...
  const users = () => {
    try {
      ...
      const userInfo: JWTPayload = JSON.parse(userInfoString); // تحويل النص إلى كائن
      setUser(userInfo);
    } catch (error) {
      console.log(error);
    }
  ...
  useEffect(() => {
    ...
    // جلب معلومات المستخدم من الـ localStorage عند تحميل الصفحة
    users();

    return () => {
      // تنظيف حدث السحب عند التخلص من المكون
      window.removeEventListener("scroll", handleStickyNavbar);
    };
  }, []); // سيتم تنفيذه مرة واحدة فقط عند تحميل المكون

  // التعامل مع تغييرات المسار بشكل صحيح
  ...
  }, [pathname]); // سيتم تنفيذه عند تغيير المسار فقط
