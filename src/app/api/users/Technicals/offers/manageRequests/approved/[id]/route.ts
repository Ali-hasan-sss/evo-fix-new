// src\app\api\users\Technicals\offers\manageRequests\approved\[id]\route.ts
...
/**
 *  @method PUT
 *  @route  ~/api/users/Technicals/offers/manageRequests/approved/:id
 *  @desc   update request to approved status
    ...
    if (user.role === "TECHNICAL") {
      ...
      const Notification  = await prisma.notification.create({
        data:{
          ...
          message: `${requestUser.user.fullName} لقد تم استلام طلبك واحالته الى قسم التحليل `
        },
        select:{
          message:true
        }
