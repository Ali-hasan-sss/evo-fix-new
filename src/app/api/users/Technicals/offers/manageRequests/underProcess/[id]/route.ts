// src\app\api\users\Technicals\offers\manageRequests\underProcess\[id]\route.ts
...
/**
 *  @method PUT
 *  @route  ~/api/users/Technicals/offers/manageRequests/underProcess/:id
 *  @desc   update request to underProcess status
    ...
    if (user.role === "TECHNICAL") {
      ...
      const Notification  = await prisma.notification.create({
        data:{
          ...
          message: `${requestUser.user.fullName} لقد تم احالة القطعة الى الصيانة سيتم اخبارك في حين الانتهاء `
        },
        select:{
          message:true
        }
